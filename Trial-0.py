...
daytopredict = "20200607000000"    #it's 2020 march 07
...
print("For {} days  Pred Temp  Real Temp   Pred %H    Real %H    Pred Rmm    Real Rmm".format(predictionLength))
...
for j in range(predictionLength):
    try:
        print("at {:>3}h {:10.1f} {:10.1f} {:10.1f} {:10.1f} {:10.1f} {:10.1f} ".format(3*j,(float((townData[indexdaythesame + j])[7])-273.16),(float((townData[indexdaytopredict + j])[7])-273.16), (float((townData[indexdaythesame + j])[9])), (float((townData[indexdaytopredict + j])[9])), (float((townData[indexdaythesame + j])[31])), (float((townData[indexdaytopredict + j])[31]))))
        tmeanError = tmeanError + abs((float((townData[indexdaythesame + j])[7])-273.16) -(float((townData[indexdaytopredict + j])[7])-273.16))
        hmeanError = hmeanError + abs((float((townData[indexdaythesame + j])[9])) -(float((townData[indexdaytopredict + j])[9])))
    except:
        tmeanError = tmeanError
        hmeanError = hmeanError
        missingData += 1
tmeanError = tmeanError/(predictionLength - missingData)
hmeanError = hmeanError/(predictionLength - missingData)
print("Absolute mean error = {:.2f} degre C  {:.1f}%H".format(tmeanError, hmeanError))